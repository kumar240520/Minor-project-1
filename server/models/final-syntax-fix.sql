-- COMPREHENSIVE FIX: Set All Required Fields for Test Data
-- This ensures ALL test data has correct type and category fields

-- ========================================
-- STEP 1: FORCE UPDATE ALL MATERIALS TYPE FIELD
-- ========================================
...
-- ========================================
-- STEP 2: ENSURE PROPER CATEGORY VALUES
-- ========================================
...
-- ========================================
-- STEP 3: CREATE MISSING PYQ MATERIALS IF NEEDED
-- ========================================
...
(gen_random_uuid(), 'PYQ - Chemistry 2023', 'Previous Year Questions for Chemistry 2023 exam with organic and inorganic chemistry', 'Chemistry', 'https://test-files.edu/pyq-chemistry-2023.pdf', (SELECT id FROM users WHERE email = 'diana.wilson@cs.test.edu' LIMIT 1), 'approved', 28, timezone('utc', now()), 40, 'pyq', 'PYQ')
ON CONFLICT DO NOTHING;

-- ========================================
-- STEP 4: CREATE MISSING REGULAR MATERIALS IF NEEDED
-- ========================================
...
SELECT 
    COUNT(*) as pyq_count,
    STRING_AGG(title, ', ' ORDER BY created_at DESC LIMIT 5) as pyq_titles
FROM materials 
WHERE type = 'pyq' 
AND status = 'approved'
...
SELECT 
    ...
    STRING_AGG(title, ', ' ORDER BY created_at DESC LIMIT 5) as material_titles
FROM materials 
WHERE type = 'material' 
AND status = 'approved'
AND uploaded_by IN (SELECT id FROM users WHERE email LIKE '%@test.edu');
...
SELECT 
    COUNT(*) as post_count,
    STRING_AGG(title, ', ' ORDER BY created_at DESC LIMIT 5) as post_titles
FROM community_posts 
WHERE user_id IN (SELECT id FROM users WHERE email LIKE '%@test.edu');

-- Show exact data structure for debugging
SELECT '=== DETAILED MATERIALS DEBUG ===' as info;
SELECT 
    id,
    title,
    type,
