...
-- Based on your actual users table structure (no avatar_url, but has avatar)
...
-- Based on your actual materials table structure (no updated_at)
INSERT INTO materials (id, title, description, subject, file_url, uploaded_by, status, downloads, views, created_at, type, file_name, file_type, uploader_name, material_type, icon_type, bg_color, text_color, storage_bucket, price, year, category) VALUES
-- Computer Science Materials
(gen_random_uuid(), 'Data Structures Complete Guide', 'Comprehensive guide to data structures including arrays, linked lists, trees, and graphs with examples and implementations', 'Data Structures', 'https://test-files.edu/data-structures.pdf', (SELECT id FROM users WHERE email = 'alice.johnson@cs.test.edu' LIMIT 1), 'approved', 45, 23, timezone('utc', now()), 'document', 'data-structures.pdf', 'PDF', 'Alice Johnson', 'study-material', 'document', 'blue', 'white', 'Storage', 10, '3rd', 'Computer Science'),
(gen_random_uuid(), 'Algorithm Design Patterns', 'Essential design patterns for software development with OOP principles and real-world applications', 'Algorithms', 'https://test-files.edu/design-patterns.pdf', (SELECT id FROM users WHERE email = 'bob.smith@cs.test.edu' LIMIT 1), 'approved', 38, 19, timezone('utc', now()), 'document', 'design-patterns.pdf', 'PDF', 'Bob Smith', 'study-material', 'document', 'green', 'white', 'Storage', 15, '4th', 'Computer Science'),
(gen_random_uuid(), 'Web Development Bootcamp', 'Complete web development course covering HTML, CSS, JavaScript, React, and Node.js', 'Web Development', 'https://test-files.edu/web-bootcamp.zip', (SELECT id FROM users WHERE email = 'charlie.brown@cs.test.edu' LIMIT 1), 'approved', 67, 31, timezone('utc', now()), 'archive', 'web-bootcamp.zip', 'ZIP', 'Charlie Brown', 'study-material', 'archive', 'purple', 'white', 'Storage', 25, '2nd', 'Computer Science'),

-- Engineering Materials
(gen_random_uuid(), 'Thermodynamics Lecture Notes', 'Complete thermodynamics notes including laws of thermodynamics, heat transfer, and practical applications', 'Thermodynamics', 'https://test-files.edu/thermodynamics.pdf', (SELECT id FROM users WHERE email = 'frank.miller@eng.test.edu' LIMIT 1), 'approved', 29, 15, timezone('utc', now()), 'document', 'thermodynamics.pdf', 'PDF', 'Frank Miller', 'study-material', 'document', 'red', 'white', 'Storage', 20, '3rd', 'Engineering'),
(gen_random_uuid(), 'Machine Shop CAD Drawings', 'Engineering CAD drawings and blueprints for various mechanical components and assemblies', 'Engineering Graphics', 'https://test-files.edu/cad-drawings.dwg', (SELECT id FROM users WHERE email = 'grace.taylor@eng.test.edu' LIMIT 1), 'approved', 51, 22, timezone('utc', now()), 'drawing', 'cad-drawings.dwg', 'DWG', 'Grace Taylor', 'study-material', 'drawing', 'orange', 'white', 'Storage', 18, '2nd', 'Engineering'),
(gen_random_uuid(), 'Circuit Analysis Lab Manual', 'Electrical engineering laboratory manual with circuit analysis techniques and practical experiments', 'Electrical Engineering', 'https://test-files.edu/circuits-lab.pdf', (SELECT id FROM users WHERE email = 'henry.anderson@eng.test.edu' LIMIT 1), 'approved', 42, 18, timezone('utc', now()), 'document', 'circuits-lab.pdf', 'PDF', 'Henry Anderson', 'study-material', 'document', 'yellow', 'white', 'Storage', 22, '4th', 'Engineering'),

-- Business Materials
(gen_random_uuid(), 'Marketing Strategy 2024', 'Complete marketing strategy framework including digital marketing, social media campaigns, and ROI analysis', 'Marketing', 'https://test-files.edu/marketing-strategy.pptx', (SELECT id FROM users WHERE email = 'kate.white@bus.test.edu' LIMIT 1), 'approved', 33, 12, timezone('utc', now()), 'presentation', 'marketing-strategy.pptx', 'PPTX', 'Kate White', 'study-material', 'presentation', 'pink', 'white', 'Storage', 30, '3rd', 'Business'),
(gen_random_uuid(), 'Financial Accounting Basics', 'Introduction to financial accounting principles, balance sheets, income statements, and cash flow analysis', 'Accounting', 'https://test-files.edu/accounting-basics.xlsx', (SELECT id FROM users WHERE email = 'liam.harris@bus.test.edu' LIMIT 1), 'approved', 28, 8, timezone('utc', now()), 'spreadsheet', 'accounting-basics.xlsx', 'XLSX', 'Liam Harris', 'study-material', 'spreadsheet', 'cyan', 'white', 'Storage', 12, '1st', 'Business'),

-- Other Materials
(gen_random_uuid(), 'Biology Lab Reports', 'Collection of biology laboratory experiments and reports covering cell biology, genetics, and ecology', 'Biology', 'https://test-files.edu/bio-labs.pdf', (SELECT id FROM users WHERE email = 'mia.garcia@sci.test.edu' LIMIT 1), 'approved', 56, 28, timezone('utc', now()), 'document', 'bio-labs.pdf', 'PDF', 'Mia Garcia', 'study-material', 'document', 'green', 'white', 'Storage', 15, '2nd', 'Science'),
(gen_random_uuid(), 'Digital Art Portfolio', 'Digital artwork collection including illustrations, graphic designs, and multimedia projects', 'Digital Arts', 'https://test-files.edu/art-portfolio.zip', (SELECT id FROM users WHERE email = 'noah.jackson@arts.test.edu' LIMIT 1), 'approved', 47, 16, timezone('utc', now()), 'archive', 'art-portfolio.zip', 'ZIP', 'Noah Jackson', 'study-material', 'archive', 'indigo', 'white', 'Storage', 20, '3rd', 'Arts'),
(gen_random_uuid(), 'Database Design Principles', 'Database design fundamentals including normalization, ER diagrams, and SQL optimization techniques', 'Database Systems', 'https://test-files.edu/database-design.pdf', (SELECT id FROM users WHERE email = 'diana.wilson@cs.test.edu' LIMIT 1), 'approved', 39, 21, timezone('utc', now()), 'document', 'database-design.pdf', 'PDF', 'Diana Wilson', 'study-material', 'document', 'blue', 'white', 'Storage', 18, '4th', 'Computer Science'),
(gen_random_uuid(), 'Mobile App Development', 'Complete guide to mobile application development using React Native and Flutter', 'Mobile Development', 'https://test-files.edu/mobile-dev.pdf', (SELECT id FROM users WHERE email = 'eva.davis@cs.test.edu' LIMIT 1), 'approved', 61, 25, timezone('utc', now()), 'document', 'mobile-dev.pdf', 'PDF', 'Eva Davis', 'study-material', 'document', 'purple', 'white', 'Storage', 28, '4th', 'Computer Science'),
(gen_random_uuid(), 'Physics Problem Sets', 'Advanced physics problems with solutions covering mechanics, electromagnetism, and quantum physics', 'Physics', 'https://test-files.edu/physics-problems.pdf', (SELECT id FROM users WHERE email = 'isabella.martinez@eng.test.edu' LIMIT 1), 'approved', 44, 17, timezone('utc', now()), 'document', 'physics-problems.pdf', 'PDF', 'Isabella Martinez', 'study-material', 'document', 'teal', 'white', 'Storage', 16, '3rd', 'Science'),
(gen_random_uuid(), 'Chemistry Lab Experiments', 'Chemistry laboratory procedures and safety guidelines for organic and inorganic chemistry', 'Chemistry', 'https://test-files.edu/chem-labs.pdf', (SELECT id FROM users WHERE email = 'jack.thomas@eng.test.edu' LIMIT 1), 'approved', 35, 14, timezone('utc', now()), 'document', 'chem-labs.pdf', 'PDF', 'Jack Thomas', 'study-material', 'document', 'brown', 'white', 'Storage', 14, '2nd', 'Science')
...
-- Based on your actual community_posts table structure (no updated_at)
...
-- Based on your actual calendar_events table structure (no updated_at)
...
-- Based on your actual transactions table structure (no reference_type, but has source)
INSERT INTO transactions (id, user_id, type, source, amount, resource_id, created_at, status, description) VALUES
-- Daily Login Rewards
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'alice.johnson@cs.test.edu' LIMIT 1), 'credit', 'daily_login', 10, NULL, timezone('utc', now() - interval '1 day'), 'completed', 'Daily login reward for consecutive 7 days'),
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'bob.smith@cs.test.edu' LIMIT 1), 'credit', 'daily_login', 10, NULL, timezone('utc', now() - interval '2 days'), 'completed', 'Daily login reward for consecutive 5 days'),
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'charlie.brown@cs.test.edu' LIMIT 1), 'credit', 'daily_login', 10, NULL, timezone('utc', now() - interval '3 days'), 'completed', 'Daily login reward for consecutive 14 days'),

-- Resource Downloads (SPEND)
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'alice.johnson@cs.test.edu' LIMIT 1), 'debit', 'material_download', -20, (SELECT id FROM materials WHERE title = 'Algorithm Design Patterns' LIMIT 1), timezone('utc', now() - interval '5 days'), 'completed', 'Downloaded Algorithm Design Patterns by Bob Smith'),
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'diana.wilson@cs.test.edu' LIMIT 1), 'debit', 'material_download', -25, (SELECT id FROM materials WHERE title = 'Web Development Bootcamp' LIMIT 1), timezone('utc', now() - interval '1 week'), 'completed', 'Downloaded Web Development Bootcamp by Charlie Brown'),
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'eva.davis@cs.test.edu' LIMIT 1), 'debit', 'material_download', -15, (SELECT id FROM materials WHERE title = 'Thermodynamics Lecture Notes' LIMIT 1), timezone('utc', now() - interval '3 days'), 'completed', 'Downloaded Thermodynamics Lecture Notes by Frank Miller'),

-- Answer Acceptance Rewards (EARN)
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'charlie.brown@cs.test.edu' LIMIT 1), 'credit', 'answer_accept', 50, NULL, timezone('utc', now() - interval '2 days'), 'completed', 'Accepted answer for helping with recursion concepts'),
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'henry.anderson@eng.test.edu' LIMIT 1), 'credit', 'answer_accept', 50, NULL, timezone('utc', now() - interval '4 days'), 'completed', 'Accepted answer for engineering internship tips'),

-- Event Attendance Rewards (EARN)
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'frank.miller@eng.test.edu' LIMIT 1), 'credit', 'event_attendance', 30, NULL, timezone('utc', now() - interval '1 day'), 'completed', 'Attended Machine Learning Workshop'),
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'kate.white@bus.test.edu' LIMIT 1), 'credit', 'event_attendance', 25, NULL, timezone('utc', now() - interval '2 weeks'), 'completed', 'Attended Tech Career Fair 2024'),

-- Fiat Purchases
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'alice.johnson@cs.test.edu' LIMIT 1), 'credit', 'fiat_purchase', 100, NULL, timezone('utc', now() - interval '1 week'), 'completed', 'Purchased 100 coins via payment gateway'),
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'bob.smith@cs.test.edu' LIMIT 1), 'credit', 'fiat_purchase', 50, NULL, timezone('utc', now() - interval '3 days'), 'completed', 'Purchased 50 coins via payment gateway')
ON CONFLICT DO NOTHING;
...
-- Based on your actual community_replies table structure (no updated_at)
INSERT INTO community_replies (id, post_id, user_id, author_name, author_role, author_avatar, content, likes, created_at) VALUES
