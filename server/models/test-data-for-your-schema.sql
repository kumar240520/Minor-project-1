...
-- Based on your actual table structure: id, email, name, full_name, role, coins, avatar_url, created_at, updated_at
...
-- Based on your actual table structure: id, user_id, resource_id, title, description, file_url, file_type, subject, semester, status, downloads, views, created_at, updated_at
INSERT INTO materials (id, user_id, resource_id, title, description, file_url, file_type, subject, semester, status, downloads, views, created_at, updated_at) VALUES
-- Computer Science Materials
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'alice.johnson@cs.test.edu' LIMIT 1), (SELECT id FROM users WHERE email = 'alice.johnson@cs.test.edu' LIMIT 1), 'Data Structures Complete Guide', 'Comprehensive guide to data structures including arrays, linked lists, trees, and graphs with examples and implementations', 'https://test-files.edu/data-structures.pdf', 'PDF', 'Data Structures', '3rd Semester', 'approved', 45, 23, timezone('utc', now()), timezone('utc', now())),
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'bob.smith@cs.test.edu' LIMIT 1), (SELECT id FROM users WHERE email = 'bob.smith@cs.test.edu' LIMIT 1), 'Algorithm Design Patterns', 'Essential design patterns for software development with OOP principles and real-world applications', 'https://test-files.edu/design-patterns.pdf', 'PDF', 'Algorithms', '4th Semester', 'approved', 38, 19, timezone('utc', now()), timezone('utc', now())),
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'charlie.brown@cs.test.edu' LIMIT 1), (SELECT id FROM users WHERE email = 'charlie.brown@cs.test.edu' LIMIT 1), 'Web Development Bootcamp', 'Complete web development course covering HTML, CSS, JavaScript, React, and Node.js', 'https://test-files.edu/web-bootcamp.zip', 'ZIP', 'Web Development', '2nd Semester', 'approved', 67, 31, timezone('utc', now()), timezone('utc', now())),

-- Engineering Materials
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'frank.miller@eng.test.edu' LIMIT 1), (SELECT id FROM users WHERE email = 'frank.miller@eng.test.edu' LIMIT 1), 'Thermodynamics Lecture Notes', 'Complete thermodynamics notes including laws of thermodynamics, heat transfer, and practical applications', 'https://test-files.edu/thermodynamics.pdf', 'PDF', 'Thermodynamics', '3rd Semester', 'approved', 29, 15, timezone('utc', now()), timezone('utc', now())),
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'grace.taylor@eng.test.edu' LIMIT 1), (SELECT id FROM users WHERE email = 'grace.taylor@eng.test.edu' LIMIT 1), 'Machine Shop CAD Drawings', 'Engineering CAD drawings and blueprints for various mechanical components and assemblies', 'https://test-files.edu/cad-drawings.dwg', 'DWG', 'Engineering Graphics', '2nd Semester', 'approved', 51, 22, timezone('utc', now()), timezone('utc', now())),
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'henry.anderson@eng.test.edu' LIMIT 1), (SELECT id FROM users WHERE email = 'henry.anderson@eng.test.edu' LIMIT 1), 'Circuit Analysis Lab Manual', 'Electrical engineering laboratory manual with circuit analysis techniques and practical experiments', 'https://test-files.edu/circuits-lab.pdf', 'PDF', 'Electrical Engineering', '4th Semester', 'approved', 42, 18, timezone('utc', now()), timezone('utc', now())),

-- Business Materials
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'kate.white@bus.test.edu' LIMIT 1), (SELECT id FROM users WHERE email = 'kate.white@bus.test.edu' LIMIT 1), 'Marketing Strategy 2024', 'Complete marketing strategy framework including digital marketing, social media campaigns, and ROI analysis', 'https://test-files.edu/marketing-strategy.pptx', 'PPTX', 'Marketing', '3rd Semester', 'approved', 33, 12, timezone('utc', now()), timezone('utc', now())),
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'liam.harris@bus.test.edu' LIMIT 1), (SELECT id FROM users WHERE email = 'liam.harris@bus.test.edu' LIMIT 1), 'Financial Accounting Basics', 'Introduction to financial accounting principles, balance sheets, income statements, and cash flow analysis', 'https://test-files.edu/accounting-basics.xlsx', 'XLSX', 'Accounting', '1st Semester', 'approved', 28, 8, timezone('utc', now()), timezone('utc', now())),

-- Other Materials
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'mia.garcia@sci.test.edu' LIMIT 1), (SELECT id FROM users WHERE email = 'mia.garcia@sci.test.edu' LIMIT 1), 'Biology Lab Reports', 'Collection of biology laboratory experiments and reports covering cell biology, genetics, and ecology', 'https://test-files.edu/bio-labs.pdf', 'PDF', 'Biology', '2nd Semester', 'approved', 56, 28, timezone('utc', now()), timezone('utc', now())),
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'noah.jackson@arts.test.edu' LIMIT 1), (SELECT id FROM users WHERE email = 'noah.jackson@arts.test.edu' LIMIT 1), 'Digital Art Portfolio', 'Digital artwork collection including illustrations, graphic designs, and multimedia projects', 'https://test-files.edu/art-portfolio.zip', 'ZIP', 'Digital Arts', '3rd Semester', 'approved', 47, 16, timezone('utc', now()), timezone('utc', now())),
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'diana.wilson@cs.test.edu' LIMIT 1), (SELECT id FROM users WHERE email = 'diana.wilson@cs.test.edu' LIMIT 1), 'Database Design Principles', 'Database design fundamentals including normalization, ER diagrams, and SQL optimization techniques', 'https://test-files.edu/database-design.pdf', 'PDF', 'Database Systems', '4th Semester', 'approved', 39, 21, timezone('utc', now()), timezone('utc', now())),
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'eva.davis@cs.test.edu' LIMIT 1), (SELECT id FROM users WHERE email = 'eva.davis@cs.test.edu' LIMIT 1), 'Mobile App Development', 'Complete guide to mobile application development using React Native and Flutter', 'https://test-files.edu/mobile-dev.pdf', 'PDF', 'Mobile Development', '4th Semester', 'approved', 61, 25, timezone('utc', now()), timezone('utc', now())),
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'isabella.martinez@eng.test.edu' LIMIT 1), (SELECT id FROM users WHERE email = 'isabella.martinez@eng.test.edu' LIMIT 1), 'Physics Problem Sets', 'Advanced physics problems with solutions covering mechanics, electromagnetism, and quantum physics', 'https://test-files.edu/physics-problems.pdf', 'PDF', 'Physics', '3rd Semester', 'approved', 44, 17, timezone('utc', now()), timezone('utc', now())),
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'jack.thomas@eng.test.edu' LIMIT 1), (SELECT id FROM users WHERE email = 'jack.thomas@eng.test.edu' LIMIT 1), 'Chemistry Lab Experiments', 'Chemistry laboratory procedures and safety guidelines for organic and inorganic chemistry', 'https://test-files.edu/chem-labs.pdf', 'PDF', 'Chemistry', '2nd Semester', 'approved', 35, 14, timezone('utc', now()), timezone('utc', now()))
...
-- Based on your actual table structure: id, user_id, title, content, category, tags, likes_count, created_at, updated_at
...
-- Based on your actual table structure: id, title, description, date, type, created_at, updated_at
...
-- Based on your actual table structure: id, user_id, reference_id, reference_type, transaction_type, amount, description, created_at
...
-- Based on your actual table structure: user_id, resource_id, purchased_at
