...
-- Fix users table - add avatar_url if missing
ALTER TABLE users ADD COLUMN IF NOT EXISTS avatar_url TEXT;

-- Fix materials table - add missing columns
...
ALTER TABLE materials ADD COLUMN IF NOT EXISTS downloads INTEGER DEFAULT 0;
ALTER TABLE materials ADD COLUMN IF NOT EXISTS views INTEGER DEFAULT 0;
ALTER TABLE materials ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT timezone('utc', now());

-- Fix community_posts table - add missing columns
ALTER TABLE community_posts ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ DEFAULT timezone('utc', now());
ALTER TABLE community_posts ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT timezone('utc', now());

-- Fix calendar_events table - add missing columns
ALTER TABLE calendar_events ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ DEFAULT timezone('utc', now());
ALTER TABLE calendar_events ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT timezone('utc', now());

-- Fix transactions table - add missing columns
ALTER TABLE transactions ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ DEFAULT timezone('utc', now());

-- Fix resource_purchases table - add missing columns
ALTER TABLE resource_purchases ADD COLUMN IF NOT EXISTS purchased_at TIMESTAMPTZ DEFAULT timezone('utc', now());

-- ========================================
-- STEP 2: CREATE 15+ TEST STUDENTS
-- ========================================
...
-- ========================================
-- STEP 3: CREATE 12 MATERIAL UPLOADS
-- ========================================
...
INSERT INTO materials (id, user_id, title, description, file_url, file_type, subject, semester, status, downloads, views, created_at, updated_at) VALUES
-- Computer Science Materials
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'alice.johnson@cs.test.edu' LIMIT 1), 'Data Structures Complete Guide', 'Comprehensive guide to data structures including arrays, linked lists, trees, and graphs with examples and implementations', 'https://test-files.edu/data-structures.pdf', 'PDF', 'Data Structures', '3rd Semester', 'approved', 45, 23, timezone('utc', now()), timezone('utc', now())),
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'bob.smith@cs.test.edu' LIMIT 1), 'Algorithm Design Patterns', 'Essential design patterns for software development with OOP principles and real-world applications', 'https://test-files.edu/design-patterns.pdf', 'PDF', 'Algorithms', '4th Semester', 'approved', 38, 19, timezone('utc', now()), timezone('utc', now())),
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'charlie.brown@cs.test.edu' LIMIT 1), 'Web Development Bootcamp', 'Complete web development course covering HTML, CSS, JavaScript, React, and Node.js', 'https://test-files.edu/web-bootcamp.zip', 'ZIP', 'Web Development', '2nd Semester', 'approved', 67, 31, timezone('utc', now()), timezone('utc', now())),
...
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'grace.taylor@eng.test.edu' LIMIT 1), 'Machine Shop CAD Drawings', 'Engineering CAD drawings and blueprints for various mechanical components and assemblies', 'https://test-files.edu/cad-drawings.dwg', 'DWG', 'Engineering Graphics', '2nd Semester', 'approved', 51, 22, timezone('utc', now()), timezone('utc', now())),
...
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'liam.harris@bus.test.edu' LIMIT 1), 'Financial Accounting Basics', 'Introduction to financial accounting principles, balance sheets, income statements, and cash flow analysis', 'https://test-files.edu/accounting-basics.xlsx', 'XLSX', 'Accounting', '1st Semester', 'approved', 28, 8, timezone('utc', now()), timezone('utc', now())),

-- Other Materials
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'mia.garcia@sci.test.edu' LIMIT 1), 'Biology Lab Reports', 'Collection of biology laboratory experiments and reports covering cell biology, genetics, and ecology', 'https://test-files.edu/bio-labs.pdf', 'PDF', 'Biology', '2nd Semester', 'approved', 56, 28, timezone('utc', now()), timezone('utc', now())),
...
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'eva.davis@cs.test.edu' LIMIT 1), 'Mobile App Development', 'Complete guide to mobile application development using React Native and Flutter', 'https://test-files.edu/mobile-dev.pdf', 'PDF', 'Mobile Development', '4th Semester', 'approved', 61, 25, timezone('utc', now()), timezone('utc', now())),
(gen_random_uuid(), (SELECT id FROM users WHERE email = 'isabella.martinez@eng.test.edu' LIMIT 1), 'Physics Problem Sets', 'Advanced physics problems with solutions covering mechanics, electromagnetism, and quantum physics', 'https://test-files.edu/physics-problems.pdf', 'PDF', 'Physics', '3rd Semester', 'approved', 44, 17, timezone('utc', now()), timezone('utc', now())),
...
-- ========================================
-- STEP 7: CREATE SAMPLE PURCHASES
-- ========================================
