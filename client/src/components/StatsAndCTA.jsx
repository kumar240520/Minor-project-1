...
const Counter = ({ from, to, duration = 2, suffix = "" }) => {
    ...
    useEffect(() => {
        if (isInView) {
            // If 'to' is already formatted (like "5K+"), use it directly
            if (typeof to === 'string' && to.includes('K')) {
                setCount(to);
                return;
            }
            
            // Otherwise animate from number to number
...
const StatsAndCTA = () => {
    ...
    const fetchPlatformStats = async () => {
        try {
            ...
            // Fetch total coins distributed to all users directly from public.users
            let totalCoins = 0;
            try {
            ...
            if (totalCoins === 0) {
                totalCoins = 80892; // Fallback value
            }
            ...
            setStats(prev => prev.map((stat, index) => {
                switch (index) {
                    case 0: return { ...stat, value: formatStat(studentCount || 0) };
                    case 1: return { ...stat, value: formatStat(materialCount || 0) };
                    case 2: return { ...stat, value: formatStat(eventCount || 0) };
                    case 3: return { ...stat, value: formatStat(totalCoins) };
                    default: return stat;
                }
            }));
        } catch (error) {
